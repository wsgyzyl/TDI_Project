---
title: "San Fransisco 311 Cases Data EDA"
...
```{r Import Downloaded Data Set, cache=TRUE}
...
```{r Import Data Set from API, eval=FALSE}
...
First, I'd like to see what is the most popular method people use to submit 311 requests and how it changes over time. geom_bar() chart is used to show the number of cases from every source from 2009 to 2018.

```{r Source Distribution Figure, echo=FALSE}
library(ggplot2)
g <- ggplot(sf311_full_year, aes(factor(Opened$year + 1900)))
g + geom_bar(aes(fill = Source))+
  labs(title = '311 Case Source Distribution Over Time') +
  xlab('Year') +
...
```{r Processing Data , message=FALSE, warning = FALSE}
library(lubridate)
library(zoo)
...
ggplot(recent_3_years, aes(monthweek, weekdayf, fill = freq)) + 
  geom_tile(colour = "white") + 
  facet_grid(year(recent_3_years$Opened.Date)~monthf) + 
